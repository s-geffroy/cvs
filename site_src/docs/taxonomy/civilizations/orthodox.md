# Orthodox (`orthodox`)

!!! warning "Avertissement éthique"
    Ce profil est inféré à partir de sources publiques agrégées. Il ne doit pas être utilisé pour classer des individus réels.

**Statut** : civilisation canonique Huntington ·
**Couverture archétypes** : complète

## Synthèse

Civilisation issue de Byzance et du christianisme orthodoxe slave, marquée par l'autocratie, le collectivisme et un héritage de modernisation tardive.

## Description longue

Distance hiérarchique très élevée (PDI souvent >85), individualisme bas-moyen (IDV 25-39), évitement d'incertitude élevé (UAI>85), restraint marqué (IVR<28). IW: TS modéré, SE négatif (survival). Inclut Russie, Ukraine, Biélorussie, Roumanie, Bulgarie, Grèce, Serbie. Frontière avec Western (Pologne, États baltes, Hongrie controversés), avec Islamic (Caucase, Balkans bosniaques).

## Cartographie académique

| Repère | Valeur |
|---|---|
| Étiquette Huntington | `Orthodox` |
| Clusters Inglehart-Welzel | Orthodox Europe |
| Quadrant IW | `TS≈0 / SE-` |
| Hofstede archétypes | RUS, UKR, ROU, BGR, GRC, SRB |


### Plages Hofstede typiques

| Dimension | Min | Max |
|---|---|---|

| PDI | 60 | 95 |

| IDV | 25 | 39 |

| MAS | 20 | 57 |

| UAI | 85 | 112 |

| LTO | 45 | 86 |

| IVR | 15 | 50 |




## Coordonnées B_vec (centroïdes)

- **`μ_viz`** (Inglehart-Welzel : TS, SE) = `[0.02, -0.72]`
- **`μ_score`** (Hofstede PDI, IDV, MAS, UAI, LTO, IVR) =
  `[83.7, 29.9, 37.9, 94.9, 66.6, 23.9]`


## États membres

| État | ISO3 | Rôle | Rationale | Citations |
|---|---|---|---|---|

| Russie | `RUS` | core | Russie, héritage byzantin via Moscou-Tiers Rome | huntington_1996 |

| Ukraine | `UKR` | core | Christianisme orthodoxe, partition uniate | huntington_1996 |

| Bélarus | `BLR` | core | Christianisme orthodoxe | huntington_1996 |

| Roumanie | `ROU` | core | Roumain orthodoxe | huntington_1996 |

| Bulgarie | `BGR` | core | Bulgare orthodoxe | huntington_1996 |

| Grèce | `GRC` | core | Grec orthodoxe, membre UE — frontière Western | huntington_1996 |

| Serbie | `SRB` | core | Serbe orthodoxe | huntington_1996 |



## Sous-clusters


- **Orthodoxe slave** (`slavic_orthodox`) — Russie (`RUS`), Ukraine (`UKR`), Bélarus (`BLR`), Serbie (`SRB`)

- **Orthodoxe balkanique** (`balkan_orthodox`) — Bulgarie (`BGR`), Roumanie (`ROU`), Grèce (`GRC`), Serbie (`SRB`)




## Cas ambigus / controverses documentées


### Ukraine (`UKR`)

- **Civilisations en concurrence** : orthodox, western
- **Discussion** : Partition uniate vs orthodoxe, alignement EU/OTAN depuis 2014/2022.
- **Citations** : huntington_1996

### Grèce (`GRC`)

- **Civilisations en concurrence** : orthodox, western
- **Discussion** : Membre UE/OTAN, mais identité orthodoxe persistante.
- **Citations** : huntington_1996



## Citations



- **[huntington_1996]** Huntington, S. P. (1996). The Clash of Civilizations and the Remaking of World Order. Simon & Schuster.



- **[inglehart_welzel_2010]** Inglehart, R., & Welzel, C. (2010). Changing Mass Priorities: The Link Between Modernization and Democracy. Perspectives on Politics, 8(2), 551-567. — [URL](https://www.cambridge.org/core/journals/perspectives-on-politics)



- **[hofstede_2010]** Hofstede, G., Hofstede, G. J., & Minkov, M. (2010). Cultures and Organizations: Software of the Mind (3rd ed.). McGraw-Hill. — [URL](https://www.hofstede-insights.com/country-comparison-tool)



- **[wvs_wave7_2022]** Haerpfer, C., Inglehart, R., Moreno, A., Welzel, C., et al. (2022). World Values Survey: Round Seven — Country-Pooled Datafile v5.0. JD Systems Institute & WVSA Secretariat. doi:10.14281/18241.20 — [URL](https://www.worldvaluessurvey.org/WVSContents.jsp)



---

[Retour à la taxonomie](../index.md) · [Méthodologie B_doc](../../methodology/08_civilizational_basis.md) · [Algèbre des distances](../../methodology/10_distance_algebra.md)
