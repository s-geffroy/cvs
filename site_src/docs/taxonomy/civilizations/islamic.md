# Islamic (`islamic`)

!!! warning "Avertissement éthique"
    Ce profil est inféré à partir de sources publiques agrégées. Il ne doit pas être utilisé pour classer des individus réels.

**Statut** : civilisation canonique Huntington ·
**Couverture archétypes** : complète

## Synthèse

Civilisation organisée autour de l'Islam comme cadre normatif et culturel, hétérogène (sunnite, chiite, soufi) et géographiquement vaste (MENA, Asie du Sud, Asie du Sud-Est).

## Description longue

IW: TS négatif (traditionnel), SE négatif (survival) — quadrant Sud-Ouest. Hofstede: PDI élevé (55-100), IDV bas (14-46), UAI moyen-élevé (36-85), restraint marqué (IVR souvent <50). Distingue sous-clusters: arabe (Maghreb + Mashreq), turco-iranien (Turquie, Iran), sud-asiatique (Pakistan, Bangladesh), sud-est asiatique (Indonésie, Malaisie). Frontières: Orthodox (Caucase), Hindic (Inde-Pakistan), Sinic (Xinjiang).

## Cartographie académique

| Repère | Valeur |
|---|---|
| Étiquette Huntington | `Islamic` |
| Clusters Inglehart-Welzel | African-Islamic |
| Quadrant IW | `TS- / SE-` |
| Hofstede archétypes | TUR, EGY, SAU, IDN, PAK, IRN |


### Plages Hofstede typiques

| Dimension | Min | Max |
|---|---|---|

| PDI | 55 | 100 |

| IDV | 14 | 46 |

| MAS | 43 | 70 |

| UAI | 36 | 85 |

| LTO | 7 | 62 |

| IVR | 0 | 57 |




## Coordonnées B_vec (centroïdes)

- **`μ_viz`** (Inglehart-Welzel : TS, SE) = `[-1.18, -0.75]`
- **`μ_score`** (Hofstede PDI, IDV, MAS, UAI, LTO, IVR) =
  `[76.1, 28.0, 51.1, 66.9, 32.5, 31.4]`


## États membres

| ISO3 | Rôle | Rationale | Citations |
|---|---|---|---|

| `TUR` | core | Turquie, héritage ottoman, identité islamique post-Atatürk | huntington_1996 |

| `IRN` | core | Iran, chiite, distinct du monde arabe sunnite | huntington_1996 |

| `EGY` | core | Égypte, centre intellectuel sunnite (Al-Azhar) | huntington_1996 |

| `SAU` | core | Arabie Saoudite, gardien des lieux saints | huntington_1996 |

| `JOR` | core | Jordanie, monarchie sunnite | huntington_1996 |

| `MAR` | core | Maroc, Maghreb arabo-berbère | huntington_1996 |

| `IRQ` | core | Irak, sunnite-chiite | huntington_1996 |

| `PAK` | core | Pakistan, État islamique sud-asiatique | huntington_1996 |

| `BGD` | core | Bangladesh, sunnite bengali | huntington_1996 |

| `IDN` | core | Indonésie, plus grand pays musulman | huntington_1996 |

| `MYS` | core | Malaisie, islam malais | huntington_1996 |



## Sous-clusters


- **Islam arabe** (`arab_islamic`) — EGY, SAU, JOR, MAR, IRQ

- **Turco-iranien** (`turco_iranian`) — TUR, IRN

- **Islam sud-asiatique** (`south_asian_islamic`) — PAK, BGD

- **Islam sud-est asiatique** (`southeast_asian_islamic`) — IDN, MYS




## Cas ambigus / controverses documentées


### `TUR`

- **Civilisations en concurrence** : islamic, western
- **Discussion** : Candidat UE depuis 1999, alliance OTAN, mais réislamisation politique. Huntington classait Turquie en État 'déchiré' (torn state).
- **Citations** : huntington_1996

### `ISR`

- **Civilisations en concurrence** : western, islamic
- **Discussion** : État juif au Moyen-Orient — ni Western ni Islamic stricto sensu.
- **Citations** : huntington_1996



## Citations



- **[huntington_1996]** Huntington, S. P. (1996). The Clash of Civilizations and the Remaking of World Order. Simon & Schuster.



- **[inglehart_welzel_2010]** Inglehart, R., & Welzel, C. (2010). Changing Mass Priorities: The Link Between Modernization and Democracy. Perspectives on Politics, 8(2), 551-567. — [URL](https://www.cambridge.org/core/journals/perspectives-on-politics)



- **[hofstede_2010]** Hofstede, G., Hofstede, G. J., & Minkov, M. (2010). Cultures and Organizations: Software of the Mind (3rd ed.). McGraw-Hill. — [URL](https://www.hofstede-insights.com/country-comparison-tool)



- **[wvs_wave7_2022]** Haerpfer, C., Inglehart, R., Moreno, A., Welzel, C., et al. (2022). World Values Survey: Round Seven — Country-Pooled Datafile v5.0. JD Systems Institute & WVSA Secretariat. doi:10.14281/18241.20 — [URL](https://www.worldvaluessurvey.org/WVSContents.jsp)



---

[Retour à la taxonomie](../index.md) · [Méthodologie B_doc](../../methodology/08_civilizational_basis.md) · [Algèbre des distances](../../methodology/10_distance_algebra.md)
