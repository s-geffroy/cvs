# Latin American (`latin_american`)

!!! warning "Avertissement éthique"
    Ce profil est inféré à partir de sources publiques agrégées. Il ne doit pas être utilisé pour classer des individus réels.

**Statut** : civilisation canonique Huntington ·
**Couverture archétypes** : complète

## Synthèse

Civilisation issue de la colonisation ibérique des Amériques, catholicisme dominant, métissage avec substrats indigènes et africains.

## Description longue

IW: TS très négatif (catholicisme persistant, valeurs traditionnelles), SE positif (post-matérialisme paradoxal lié à la chaleur sociale, à l'autonomie individuelle). Hofstede: PDI élevé (49-95), IDV bas (6-46), UAI très élevé (76-101), IVR variable (mais souvent élevé). Sous-clusters: Cône Sud (ARG, CHL, URY), Mexique/Amérique centrale, Andin, Brésil. Frontière avec Western (héritage ibérique partagé) et Indigenous (BOL, ECU, PER).

## Cartographie académique

| Repère | Valeur |
|---|---|
| Étiquette Huntington | `Latin American` |
| Clusters Inglehart-Welzel | Catholic Latin America |
| Quadrant IW | `TS- / SE+` |
| Hofstede archétypes | BRA, MEX, ARG, CHL, COL |


### Plages Hofstede typiques

| Dimension | Min | Max |
|---|---|---|

| PDI | 49 | 95 |

| IDV | 6 | 46 |

| MAS | 28 | 73 |

| UAI | 76 | 101 |

| LTO | 13 | 44 |

| IVR | 46 | 100 |




## Coordonnées B_vec (centroïdes)

- **`μ_viz`** (Inglehart-Welzel : TS, SE) = `[-0.76, 0.74]`
- **`μ_score`** (Hofstede PDI, IDV, MAS, UAI, LTO, IVR) =
  `[70.0, 24.4, 50.7, 85.9, 28.8, 69.8]`


## États membres

| État | ISO3 | Rôle | Rationale | Citations |
|---|---|---|---|---|

| Brésil | `BRA` | core | Brésil, héritage portugais | huntington_1996 |

| Mexique | `MEX` | core | Mexique, héritage espagnol + substrat mésoaméricain | huntington_1996 |

| Argentine | `ARG` | core | Argentine, Cône Sud, fortement européanisé | huntington_1996 |

| Chili | `CHL` | core | Chili, Cône Sud | huntington_1996 |

| Colombie | `COL` | core | Colombie, andine | huntington_1996 |

| Pérou | `PER` | core | Pérou, andin + substrat quechua-aymara | huntington_1996 |

| Uruguay | `URY` | core | Uruguay, Cône Sud, laïcisé | huntington_1996 |

| Venezuela | `VEN` | core | Venezuela, caribéen | huntington_1996 |

| Guatemala | `GTM` | core | Guatemala, méso-américain + substrat maya | huntington_1996 |

| Bolivie | `BOL` | interface | Bolivie, identité plurinationale (latino + aymara/quechua/guarani) | huntington_1996, smith_2012 |

| Équateur | `ECU` | interface | Équateur, identité métisse + indigène (Kichwa) | huntington_1996, smith_2012 |



## Sous-clusters


- **Cône Sud** (`southern_cone`) — Argentine (`ARG`), Chili (`CHL`), Uruguay (`URY`)

- **Andin** (`andean`) — Colombie (`COL`), Pérou (`PER`), Bolivie (`BOL`), Équateur (`ECU`)

- **Mexique / Amérique centrale** (`mexican_central_american`) — Mexique (`MEX`), Guatemala (`GTM`)

- **Brésil + Caraïbes** (`brazil_caribbean`) — Brésil (`BRA`), Venezuela (`VEN`)




## Cas ambigus / controverses documentées


### Mexique (`MEX`)

- **Civilisations en concurrence** : latin_american, western
- **Discussion** : Membre USMCA, intégration économique nord-américaine, mais identité culturelle latino-américaine forte.
- **Citations** : huntington_1996

### Bolivie (`BOL`)

- **Civilisations en concurrence** : latin_american, indigenous
- **Discussion** : Constitution 2009 reconnaît État plurinational. Identité indigène majoritaire dans certaines régions.
- **Citations** : smith_2012, un_unpfii



## Citations



- **[huntington_1996]** Huntington, S. P. (1996). The Clash of Civilizations and the Remaking of World Order. Simon & Schuster.



- **[inglehart_welzel_2010]** Inglehart, R., & Welzel, C. (2010). Changing Mass Priorities: The Link Between Modernization and Democracy. Perspectives on Politics, 8(2), 551-567. — [URL](https://www.cambridge.org/core/journals/perspectives-on-politics)



- **[hofstede_2010]** Hofstede, G., Hofstede, G. J., & Minkov, M. (2010). Cultures and Organizations: Software of the Mind (3rd ed.). McGraw-Hill. — [URL](https://www.hofstede-insights.com/country-comparison-tool)



- **[wvs_wave7_2022]** Haerpfer, C., Inglehart, R., Moreno, A., Welzel, C., et al. (2022). World Values Survey: Round Seven — Country-Pooled Datafile v5.0. JD Systems Institute & WVSA Secretariat. doi:10.14281/18241.20 — [URL](https://www.worldvaluessurvey.org/WVSContents.jsp)



---

[Retour à la taxonomie](../index.md) · [Méthodologie B_doc](../../methodology/08_civilizational_basis.md) · [Algèbre des distances](../../methodology/10_distance_algebra.md)
